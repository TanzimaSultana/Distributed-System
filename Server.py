...
# Reserve a port number server for connection
...
class server:
    ...
    def create_server_gui(self):
        ...
        self.serverStatus = tk.Label(self.master, text = "Server Status : Ready")
    ...
    def server_thread(self): 
        ...
        try:
            ...
            pollingSocket.bind(('', POLLING_PORT)) 
            ...
            # Backup server
            backupSocket = self.init_backup_server()
            
            while True:
                try:
                    print("server_thread : bind to client successful")
                    ...
                    # Start client thread
                    # Param = connection & backup socket
                    thread = threading.Thread(target=self.client_thread, args=(connectionSocket,backupSocket,))
                    thread.start()
    ...
    ### -------------------------- Client Thread --------------------------- ###
    # For each client
    def client_thread(self, conn_socket, backup_scoket):
    ...
    # Disconnect client
    def disconnect_client(self, conn_socket, backup_scoket):
    ...
    ### -------------------------- Backup server --------------------------- ###
    def init_backup_server(self):
        # https://stackoverflow.com/questions/1886090/return-value-from-thread
        # Send queue as args, to return connection status & backup socket from thread
        ret_queue = Queue(maxsize = 0)
        thread = threading.Thread(target = self.open_backup_server_connection,args=[ret_queue],)
        thread.start()
        thread.join()
        
        # Get connection status & backup socket from queue, FIFO queue
        conn_msg = ret_queue.get()
        backup_socket = ret_queue.get()
        
        if(conn_msg == SUCCESSFUL and backup_socket != None):
            # Send the lexicon file to the backup server
            # So at start, Primary & Backup server will have same lexicon words
            self.update_backup_server(backup_socket, BACKUP_LEXICON_WORDS)
            return backup_socket
        else:
            return None
            
    # Connect to backup server, return backup connection socket & msg
    def open_backup_server_connection(self, ret_queue):
        conn_msg = FAILED
        backup_socket = socket.socket()
        try:
            # Connect to the backup server
            backup_socket.connect_ex((SERVER_ADDR, BACKUP_SERVER_PORT))
            conn_msg = SUCCESSFUL
        except Exception as e:
            print("open_backup_server_connection : Exception: ", str(e))
            conn_msg = FAILED
            
        ret_queue.put(conn_msg)
        ret_queue.put(backup_socket)
    
    # Update backup server
    def update_backup_server(self, backup_scoket, key):
        # Fork a thread to update backup server
        thread = threading.Thread(target=self.backup_server_thread, args=(backup_scoket,key,))
        thread.start()
    
    # Send & receive from backup server
    def backup_server_thread(self, backup_scoket,key):
        recv_msg = FAILED
        send_msg = FAILED
        
        # Open file, read from it and send to backup server
        # https://gist.github.com/giefko/2fa22e01ff98e72a5be2
        filename = FILE_PATH + "lexicon_file.txt"
        file = open(filename,'rb')
        txt = file.read(1024)
        while (txt):
            # Send data 
            send_msg = self.send_data_to(txt, key, backup_scoket)
            # Read from file
            txt = file.read(1024)
        # Close file
        file.close()
              
        if(send_msg == SUCCESSFUL):
            recv_data = self.recv_data_from(backup_scoket)
            if(KEY_MESSAGE in recv_data):
                recv_msg = recv_data[KEY_MESSAGE]
                
        print("backup_server_thread : ", key, send_msg, recv_msg)
